...
class TestRT(
    ...
    companion object {
        ...
        fun getNextTrialModes(ctx: Context):List<List<Int>>{
            return if(VibrationManager.sysHasVibrator(ctx))
                listOf( listOf(TEST_NEXTTRIAL_NOCHOOSE), listOf(TEST_NEXTTRIAL_NOCHOOSE), listOf(TEST_NEXTTRIAL_NOCHOOSE))
            else
    ...
    override var mDrawablesResource: MutableList<Int> = mutableListOf(R.drawable.blue_circle)
    ...
    private fun createTrials():List<TrialBasic>{
        ...
        for(i in 0 until NUM_TRIALS)
            trials.add(TrialBasic(i, subject.type, mTestLabel))

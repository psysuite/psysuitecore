...
class ATVBUnBalancedSummary(ctx:Context) : Summary(ctx){
    ...
    private val cond_labels:List<String> = listOf("A_TV", "T_AV", "V_AT")
    ...
    inner class ATVBsummaryCondition(val type:Int, val label:String){

        private var latencies:MutableList<ATVBsummaryRow> = when(type) {
            A_TV -> mutableListOf(
    ...
    inner class ATVBsummaryRow(val type:Int, val label:String, val latency:Int){
        ...
        fun close(){
            ...
            if(ntrial > 0){
                ...
                perc_succ           = ((perc_succ / ntrial) * 100F).roundToInt()
                perc_discrimination = ((perc_discrimination / ntrial) * 100F).roundToInt()
            }
        }
